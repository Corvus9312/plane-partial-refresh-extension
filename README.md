...
4. 「回來自動刷新」：切回 Plane 分頁時，若距上次刷新已超過設定間隔才再刷（預設 1 分鐘；手動或定時刷新也算上次）
5. 「定時刷新」與「回來自動刷新」**只在 work items 頁**（網址含 `/projects/.../issues`）會觸發；在 cycles、modules、設定等其它頁面不會自動刷。手動點圖示不受此限
6. 「定時刷新」與「回來自動刷新」可同時開，互不衝突
